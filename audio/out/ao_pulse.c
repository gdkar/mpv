...
static int wait_audio(struct ao *ao, pthread_mutex_t *lock)
{
    ...
    pthread_mutex_lock(&priv->wakeup_lock);
    while (!priv->wakeup_status) pthread_cond_wait(&priv->wakeup, &priv->wakeup_lock);
...
#endif
    default:
        if (af_fmt_is_spdif(format)) return PA_ENCODING_ANY;
...
{
    if (src->num > PA_CHANNELS_MAX) return false;
    ...
    for (int n = 0; n < src->num; n++) {
        ...
        if (pa_speaker == PA_CHANNEL_POSITION_INVALID) return false;
...
{
    struct priv *priv = ao->priv;
    if (priv->mainloop) pa_threaded_mainloop_stop(priv->mainloop);
    if (priv->stream) {
...
static int pa_init_boilerplate(struct ao *ao)
{
    ...
    if (pa_threaded_mainloop_start(priv->mainloop) < 0)  goto fail;
    pa_threaded_mainloop_lock(priv->mainloop);
    locked = true;
    ...
    if (pa_context_connect(priv->context, host, 0, NULL) < 0) goto fail;
    /* Wait until the context is ready */
    while (1) {
        int state = pa_context_get_state(priv->context);
        if (state == PA_CONTEXT_READY)  break;
        if (!PA_CONTEXT_IS_GOOD(state)) goto fail;
        pa_threaded_mainloop_wait(priv->mainloop);
    }
    pa_threaded_mainloop_unlock(priv->mainloop);
    return 0;
fail:
    if (locked) pa_threaded_mainloop_unlock(priv->mainloop);
    if (priv->context) {
        pa_threaded_mainloop_lock(priv->mainloop);
...
static int init(struct ao *ao)
{
    ...
    if (pa_init_boilerplate(ao) < 0) return -1;
    pa_threaded_mainloop_lock(priv->mainloop);
    if (!(format = pa_format_info_new())) goto unlock_and_fail;
    ...
    if (!select_chmap(ao, &map)) goto unlock_and_fail;
    ...
    while (1) {
        ...
        if (!PA_STREAM_IS_GOOD(state)) goto unlock_and_fail;
        pa_threaded_mainloop_wait(priv->mainloop);
    }
...
unlock_and_fail:
    pa_threaded_mainloop_unlock(priv->mainloop);
    if (format)   pa_format_info_free(format);
...
static void reset(struct ao *ao)
{
    ...
    pa_threaded_mainloop_lock(priv->mainloop);
    priv->retval = 0;
    if (!waitop(priv, pa_stream_flush(priv->stream, success_cb, ao)) || !priv->retval)
...
static double get_delay_hackfixed(struct ao *ao)
{
    ...
    if (sink_latency > 0) latency += sink_latency;
...
{
    struct priv *priv = ao->priv;
    if (priv->cfg_latency_hacks) { return get_delay_hackfixed(ao);}
    else {return get_delay_pulse(ao);}
}
