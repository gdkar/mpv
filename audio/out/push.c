...
static void wakeup_playthread(struct ao *ao)
{
    ...
    if (ao->driver->wakeup) ao->driver->wakeup(ao);
    p->need_wakeup = true;
...
static void reset(struct ao *ao)
{
    ...
    p->paused = false;
    if (p->still_playing) wakeup_playthread(ao);
...
static void audio_pause(struct ao *ao)
{
    struct ao_push_state *p = ao->api_priv;
    pthread_mutex_lock(&p->lock);
    if (ao->driver->pause) ao->driver->pause(ao);
    p->paused = true;
...
static void resume(struct ao *ao)
{
    struct ao_push_state *p = ao->api_priv;
    pthread_mutex_lock(&p->lock);
    if (ao->driver->resume) ao->driver->resume(ao);
    p->paused = false;
...
static int play(struct ao *ao, void **data, int samples, int flags)
{
    ...
    if (write_samples < samples) flags = flags & ~AOPLAY_FINAL_CHUNK;
    bool is_final = flags & AOPLAY_FINAL_CHUNK;
    struct mp_audio audio;
    mp_audio_buffer_get_format(p->buffer, &audio);
    for (int n = 0; n < ao->num_planes; n++) audio.planes[n] = data[n];
    ...
    if (got_data) wakeup_playthread(ao);
    pthread_mutex_unlock(&p->lock);
...
static void ao_play_data(struct ao *ao)
{
    ...
    space = MPMAX(space, 0);
    if (data.samples > space) data.samples = space;
    int flags = 0;
    if (p->final_chunk && data.samples == max) flags |= AOPLAY_FINAL_CHUNK;
    MP_STATS(ao, "start ao fill");
    int r = 0;
    if (data.samples) r = ao->driver->play(ao, data.planes, data.samples, flags);
    ...
    if (stuck) MP_ERR(ao, "Audio output is reporting incorrect buffer status.\n");
    ...
    if (more) mp_input_wakeup(ao->input_ctx); // request more data
            ...
            if (!p->wait_on_ao || p->paused) {
                    ...
                    !mp_audio_buffer_samples(p->buffer))
                {
                    ...
                    if (p->expected_end_time < now) {
                        p->still_playing = false;
                    } else {timeout = p->expected_end_time - now;}
                }
                if (was_playing && !p->still_playing) mp_input_wakeup(ao->input_ctx);
...
static void destroy_no_thread(struct ao *ao)
{
    struct ao_push_state *p = ao->api_priv;
    ao->driver->uninit(ao);
    for (int n = 0; n < 2; n++) close(p->wakeup_pipe[n]);
...
static int init(struct ao *ao)
{
    ...
    mp_audio_buffer_reinit_fmt(p->buffer, ao->format,&ao->channels, ao->samplerate);
    mp_audio_buffer_preallocate_min(p->buffer, ao->buffer);
    if (pthread_create(&p->thread, NULL, playthread, ao)) goto err;
    return 0;
err:
    destroy_no_thread(ao);
    return -1;
...
int ao_play_silence(struct ao *ao, int samples)
{
    ...
    if (samples <= 0 || !af_fmt_is_pcm(ao->format) || !ao->driver->play) return 0;
...
int ao_wait_poll(struct ao *ao, struct pollfd *fds, int num_fds,pthread_mutex_t *lock)
{
    ...
    assert(ao->api == &ao_api_push);
    assert(&p->lock == lock);
    if (num_fds >= MAX_POLL_FDS || p->wakeup_pipe[0] < 0) return -1;
    ...
    if (p_fds[num_fds].revents & POLLIN) {
        ...
        char buf[100];
        int bytes = read(p->wakeup_pipe[0], buf, sizeof(buf));
...
void ao_wakeup_poll(struct ao *ao)
{
    ...
    int ret;
    do{
      ret = write(p->wakeup_pipe[1], &(char){0}, 1);
