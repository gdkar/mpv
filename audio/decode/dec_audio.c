...
struct mp_decoder_list *audio_decoder_list(void)
{
    ...
    for (int i = 0; ad_drivers[i] != NULL; i++) ad_drivers[i]->add_decoders(list);
    return list;
}
static struct mp_decoder_list *audio_select_decoders(struct dec_audio *d_audio)
{
    ...
    struct mp_decoder_list *new = mp_select_decoders(list, codec, opts->audio_decoders);
...
static const struct ad_functions *find_driver(const char *name)
{
    for (int i = 0; ad_drivers[i] != NULL; i++) {
        if (strcmp(ad_drivers[i]->name, name) == 0) return ad_drivers[i];
    }
    return NULL;
}
int audio_init_best_codec(struct dec_audio *d_audio)
{
    ...
    for (int n = 0; n < list->num_entries; n++) {
        ...
        MP_WARN(d_audio, "Audio decoder init failed for ""%s:%s\n", sel->family, sel->decoder);
    }
    if (d_audio->ad_driver) {
...
static bool copy_output(struct af_stream *afs, struct mp_audio_buffer *outbuf,int minsamples, bool eof)
{
    while (mp_audio_buffer_samples(outbuf) < minsamples) {
        if (af_output_frame(afs, eof) < 0) return true; // error, stop doing stuff
        struct mp_audio *mpa = af_read_output_frame(afs);
        if (!mpa) return false; // out of data
...
int audio_decode(struct dec_audio *da, struct mp_audio_buffer *outbuf,int minsamples)
{
    struct af_stream *afs = da->afilter;
    if (afs->initialized < 1) return AD_ERR;
    MP_STATS(da, "start audio");
    int res;
    while (1) {
        res = 0;
        if (copy_output(afs, outbuf, minsamples, false)) break;
        ...
        da->waiting = NULL;
        if (af_filter_frame(afs, mpa) < 0) return AD_ERR;
    }
...
{
    if (d_audio->ad_driver) d_audio->ad_driver->control(d_audio, ADCTRL_RESET, NULL);
